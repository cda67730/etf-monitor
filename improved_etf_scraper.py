...
def main():
    ...
    def should_run():
        ...
        return today < 5  # 0-4 表示週一到週五
    
    def scheduled_task():
        """定時任務"""
        if should_run():
            logger.info("今天是工作日，執行爬取任務")
            scraper.scrape_all_etfs()
        else:
            logger.info("今天是週末，跳過爬取任務")
    
    # 立即執行一次（如果是工作日）
    logger.info("程式啟動")
    if should_run():
        logger.info("立即執行一次爬取")
        scraper.scrape_all_etfs()
    
    # 設置定時任務 - 每天晚上9:30執行
    schedule.every().day.at("21:30").do(scheduled_task)
    
    logger.info("定時任務已設置，每天晚上21:30執行（僅週一至週五）")
    logger.info("程式運行中... (按Ctrl+C停止)")
...
if __name__ == "__main__":
    # 可以先測試單個請求
    scraper = ETFHoldingsScraper()
    test_result = scraper.test_single_request('00981A')
    
    if test_result:
        logger.info("測試成功，啟動主程式")
        main()
    else:
        logger.error("測試失敗，請檢查API或網路連線")